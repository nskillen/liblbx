 ...
 **/
LBXFile LBXFile::create_lbx(std::string filename, std::vector<std::string> files_to_pack, uint16_t version = 0) {
	LBXFile lbx;
...
const size_t LBXFile::size() const {
	size_t file_size = 0;
...
const size_t LBXFile::num_packed_files() const {
	return files.size();
}
